...
export const signup = async (req, res) => {
  let errors = [];
  const { name, email, tipo, grado, paralelo, password, confirm_password } = req.body;
  ...
  if (errors.length > 0) {
    return res.render("auth/signup", {
      errors, name, email, tipo, grado, paralelo, password, confirm_password,
  ...
  // Saving a New User
  const newUser = new User({ name, email,tipo, grado, paralelo, password });
